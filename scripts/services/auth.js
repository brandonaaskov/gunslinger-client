(function() {
  angular.module('gunslinger').service('auth', function($firebase, $firebaseSimpleLogin, $cookies, config, alerts) {
    var auth, getCurrentUser, getUser, hasAccount, login, updateBasic, updateComplete, updateUser, user;
    ...
    getUser = function(guid) {
      return $firebase(new Firebase("" + config.firebase.users + "/basic/" + $cookies.guid));
    };
